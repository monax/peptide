...
type Builder struct {
	...
	EnumInfos []impl.EnumInfo

	// MessageInfos is a list of message infos in "flattened ordering".
	// If provided, the GoType and PBType for each element is populated.
	//
	// Requirement: len(MessageInfos) == len(Build.Messages)
	MessageInfos []impl.MessageInfo

	// ExtensionInfos is a list of extension infos in "flattened ordering".
	// Each element is initialized and registered with the protoregistry package.
	//
	// Requirement: len(LegacyExtensions) == len(Build.Extensions)
	ExtensionInfos []impl.ExtensionInfo
...
func (tb Builder) Build() (out Out) {
	...
	for i := range fbOut.Extensions {
		...
		impl.InitExtensionInfo(&tb.ExtensionInfos[i], &fbOut.Extensions[i], goType)
...
func (r *resolverByIndex) FindEnumByIndex(i, j int32, es []filedesc.Enum, ms []filedesc.Message) protoreflect.EnumDescriptor {
	if depIdx := int(r.depIdxs.Get(i, j)); int(depIdx) < len(es)+len(ms) {
		...
		return impl.Export{}.EnumDescriptorOf(r.goTypes[depIdx])
	}
}
...
func (r *resolverByIndex) FindMessageByIndex(i, j int32, es []filedesc.Enum, ms []filedesc.Message) protoreflect.MessageDescriptor {
	if depIdx := int(r.depIdxs.Get(i, j)); depIdx < len(es)+len(ms) {
		...
		return impl.Export{}.MessageDescriptorOf(r.goTypes[depIdx])
	}
}
