	...
	for len(b) > 0 {
		...
		case protowire.VarintType:
			...
			case genid.FileDescriptorProto_PublicDependency_field_number:
				fd.L2.Imports.slice[v].IsPublic = true
			case genid.FileDescriptorProto_WeakDependency_field_number:
				fd.L2.Imports.slice[v].IsWeak = true
			}
		case protowire.BytesType:
			...
			case genid.FileDescriptorProto_Dependency_field_number:
				...
				if imp == nil {
					imp = NewPlaceholderFile(path)
				}
				fd.L2.Imports.slice = append(fd.L2.Imports.slice, pref.FileImport{FileDescriptor: imp})
...
func (fd *Field) unmarshalFull(b []byte, sb *strs.Builder, pf *File, pd pref.Descriptor, i int) {
	...
	if rawTypeName != nil {
		...
		switch fd.L1.Kind {
		case pref.EnumKind:
			fd.L1.Enum = NewPlaceholderEnum(name)
		case pref.MessageKind, pref.GroupKind:
			fd.L1.Message = NewPlaceholderMessage(name)
		}
	}
...
func (xd *Extension) unmarshalFull(b []byte, sb *strs.Builder) {
	...
	if rawTypeName != nil {
		...
		switch xd.L1.Kind {
		case pref.EnumKind:
			xd.L2.Enum = NewPlaceholderEnum(name)
		case pref.MessageKind, pref.GroupKind:
			xd.L2.Message = NewPlaceholderMessage(name)
		}
	}
	...
	for len(b) > 0 {
		...
		case protowire.BytesType:
			...
			case genid.MethodDescriptorProto_InputType_field_number:
				md.L1.Input = NewPlaceholderMessage(makeFullName(sb, v))
			case genid.MethodDescriptorProto_OutputType_field_number:
				md.L1.Output = NewPlaceholderMessage(makeFullName(sb, v))
