...
// NewConverter matches a Go type with a protobuf field and returns a peptide.Converter
...
func NewConverter(t reflect.Type, fd pref.FieldDescriptor) peptide.Converter {
	switch {
	case fd.IsList():
...
func newSingularConverter(t reflect.Type, fd pref.FieldDescriptor) peptide.Converter {
...
func newEnumConverter(goType reflect.Type, fd pref.FieldDescriptor) peptide.Converter {
	var def pref.Value
	if fd.Cardinality() == pref.Repeated {
...
func newMessageConverter(goType reflect.Type) peptide.Converter {
...
func (c *messageConverter) PBValueOf(v reflect.Value) pref.Value {
	...
	panic(fmt.Errorf("%v is not ProtoMessge", v.Type()))
}
