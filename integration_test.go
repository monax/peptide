		...
		t.Run("BuildRelease", func(t *testing.T) {
			...
			for _, goos := range []string{"linux", "darwin", "windows"} {
				for _, goarch := range []string{"386", "amd64"} {
					...
					binPath := filepath.Join("bin", fmt.Sprintf("protoc-gen-go-peptide.%v.%v.%v", v, goos, goarch))

					// Build the binary.
					cmd := command{Env: append(os.Environ(), "GOOS="+goos, "GOARCH="+goarch)}
					cmd.mustRun(t, "go", "build", "-trimpath", "-ldflags", "-s -w -buildid=", "-o", binPath, "./cmd/protoc-gen-go-peptide")
					...
					gz.Comment = fmt.Sprintf("protoc-gen-go-peptide VERSION=%v GOOS=%v GOARCH=%v", v, goos, goarch)
					tw := tar.NewWriter(gz)
					tw.WriteHeader(&tar.Header{
						Name: "protoc-gen-go-peptide",
						Mode: int64(0775),
						Size: int64(len(in)),
					})
					tw.Write(in)
